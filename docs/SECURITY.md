...
### GitHub issue tasks and draft-PR publication

- GitHub content and credentials cross Forge's boundary only through the explicit `--from-issue` and `--pr` flags. Both drive the `gh` CLI through the same bounded shell-free executor as every other command: Forge-composed token arrays, a 30-second timeout, and a 200,000-character output bound. Forge never reads, parses, or stores a GitHub token; only `GH_TOKEN`/`GITHUB_TOKEN` are forwarded into Forge's own `gh` invocations, and the model-command environment allowlist is unchanged. `gh` is not part of the model's command vocabulary.
- `--from-issue` accepts `<n>`, `#<n>`, `owner/repo#<n>`, or a `github.com` issue URL; other hosts, pull URLs, and query/fragment additions are refused. The fetched issue is untrusted task text: title and body are hard-bounded (300/12,000 characters with an explicit truncation marker) and composed deterministically into the same place a user-typed task would occupy — never into a command. An issue author can still instruct the model through that text; with `--yes` this prompt-injection residual is real, and the ordinary approval, sandbox, verification, and risk-scan mechanisms are the mitigation, not a solution.
- `--pr` requires `--isolate`, is incompatible with `--no-verify`, and publishes only after the verification gate passes, the patch is captured, and the same risk decision that gates `--promote` allows it (`--allow-risk` is the same explicit override). Publication commits in the detached worktree under the repository's own git identity (unset identity is a hard error, patch retained), refuses without an `origin` remote, refuses if `forge/<sessionId>` already exists on origin, pushes exactly one fresh `forge/<sessionId>` branch — never `--force`, structurally never the default branch — and opens a draft PR whose body references the issue plainly without autoclose keywords. Every composed git/gh argv and its outcome is recorded in the result document's `github` metadata block. The one partial state — branch pushed but PR creation failed — reports the branch and never deletes or retries it. `gh` network egress happens on the host, consistent with Git operations, even when a container backend is configured.
...
- MCP beyond the opt-in stdio client: no HTTP/SSE/WebSocket transports, no resources/prompts/sampling, no sandboxing of the server process (it runs on the host under the scrubbed environment allowlist), and no config trust pinning — forge.json is model-editable, so a planted `mcp.servers` command would start for a user who habitually passes `--mcp`;
