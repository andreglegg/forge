...
| Change-impact planning | Shipped | Actual committed paths are mapped through bounded inbound dependency closure to owning packages and candidate tests. The plan is fed into the next model turn and retained in headless JSON results; it guides iteration but never replaces the authoritative completion gate. Deleted or moved-away paths are reported as unanalyzable rather than guessed. |
