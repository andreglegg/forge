...
`src/impact.ts` derives a bounded post-mutation impact plan from actual committed paths. It walks the static inbound dependency closure, identifies nearest package manifests, ranks directly affected and same-package tests, reports deleted or moved-away paths as unanalyzable, feeds the plan into the next model turn, and retains it in headless results. This is planning evidence only; the configured authoritative completion gate remains mandatory.
