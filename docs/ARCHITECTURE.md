...
The current TypeScript product does not yet provide automatic caller/dependency-backed context selection, full language-server references, dynamic-dispatch resolution, path-alias or package-export resolution, change-aware test selection, an OS-level sandbox, MCP, a general plugin API, remote workers, interactive lifecycle hooks, or a full-screen TUI. These are roadmap items and must not be inferred from historical documents.
