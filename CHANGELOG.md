...
- Post-mutation change-impact planning from actual committed paths, including bounded inbound dependency closure, owning packages, candidate tests, next-turn model evidence, and retained headless JSON results. Deleted or moved-away paths are reported as unanalyzable rather than guessed; the authoritative completion gate remains unchanged.
